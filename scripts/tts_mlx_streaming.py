...
def log(level: str, msg: str):
    # Write logs to stderr when stdout is in binary mode
    print(make_log(level, msg), file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        "out", type=str, help="Output file to generate, use - for playing the audio, use stdout for raw PCM streaming"
    )
    parser.add_argument(
        "--hf-repo",
        type=str,
    ...
    # Set up unbuffered binary stdout for low latency streaming
    if args.out == "stdout":
    ...
    # Only create queue if not using stdout mode
    wav_frames = None if args.out == "stdout" else queue.Queue()
    ...
    def _on_frame(frame):
        ...
        if args.out == "stdout":
            # Write PCM directly to stdout as float32 binary data for ultra-low latency
            # Ensure little-endian byte order for consistent playback
            pcm_bytes = _pcm.astype('<f4').tobytes()  # <f4 = little-endian float32
            sys.stdout.write(pcm_bytes)
        else:
    ...
    def run():
        ...
        for line in sys.stdin:
            ...
            log("info", f"processing new message: {line_text[:50]}...")
            
            # Create a completely fresh TTSGen instance for each message
            # This eliminates any possibility of state corruption between messages
            gen = TTSGen(tts_model, all_attributes, on_frame=_on_frame)
            
            entries = prepare_script(tts_model, line_text, first_turn=first_turn)
            first_turn = False
            for entry in entries:
                gen.append_entry(entry)
                gen.process()
            
            # Process remaining entries for this message
            gen.process_last()
            log("info", "message processing complete")

    if args.out == "stdout":
        # Direct stdout streaming mode - no buffering, ultra-low latency
        run()
    elif args.out == "-":
        # Audio playback mode
